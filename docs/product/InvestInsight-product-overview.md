...
当前主界面已经向 2.0 设计稿落地，并开始接入 v2.2 内容可读性优化：整体采用左侧导航、顶部状态条和内容工作区，左侧入口包含“总览、事件雷达、板块机会、策略跟踪、AI 助手、配置”。顶部状态条只展示当前页面标题、说明和运行状态，不再保留 AI 助手/配置快捷按钮，也不再显示右侧页签条；“开始分析”和 AI 开关位于总览页的分析控制卡片中。总览页已经委托独立 `DashboardRenderer`，事件雷达页已经委托独立 `EventRadarRenderer`，板块机会页已经委托独立 `SectorTableRenderer`，策略跟踪页已经委托独立 `StrategyRenderer`，板块详情页已经委托独立 `SectorDetailRenderer`，指数详情页已经委托独立 `IndexDetailRenderer`。事件雷达已展示结构化事件时间线、事件状态、观察点、证据可信度、事件催化分和传导路径；总览页已把关键事件和板块机会拆成可读标题、首要理由、首要风险和下一观察点；板块机会页默认使用专业模式，列表删除“风险提示”列，改为展示 MACD、RSI、KDJ、均线、量能、资金、数据质量、点位计划和主要看点；策略页新增跟踪状态卡片、推荐跟踪/信号复盘，并把事件列表收束为策略验证日历；AI 助手已改为当前上下文 + 快捷问题 + 对话区；配置页已嵌入工作台，并同屏展示 AI 接入、我的持仓、后台刷新与提醒、数据源健康；板块详情页已改为“决策摘要、趋势图与点位、购买建议、相关事件分析、预测矩阵、证据层”的聚焦布局，首屏优先展示行动判断、买入观察区、止损失效位、止盈减仓区和风险收益比，证据层继续保留核心评分、AI 协同解读、技术指标、资金流、回测、新闻证据和数据质量。
...
板块详情页是产品的关键决策页，视觉优化不能只保留事件解释。当前详情页已按“先决策、再证据”的方式组织：上半部分先给出行动判断、趋势图、点位计划、相关事件和短中长期预测，下半部分再展示核心评分、AI 协同解读、信号解释、影响路径、阶段收益/回测、资金流、技术指标、新闻证据和数据质量，让用户既能快速判断是否值得行动，也能继续核对原始证据。
...
- 可选 AI 两阶段分析：先做新闻影响归因，再对重点板块做深度研判；v2.2 起 AI 会额外输出可读标题、摘要、影响路径、首要理由/风险和规则分歧提示，但只作为解释增强，不直接覆盖核心评分或最终动作。
- 市场环境识别，包括指数、涨跌家数、资金流和风险偏好状态。
- 板块趋势、技术指标、轮动、过热、资金结构和解释链分析。
- v2.4 技术点位计划：基于 K 线、均线、BOLL、ATR、风险收益比和趋势状态，生成观察买入区、止损失效位、止盈减仓区和持有周期，并在推荐生命周期中校正入场时机。
...
该命令会输出事件类型、状态、地区、观察节点、时间字段、证据可信度、影响周期、评分因子，以及黄金、有色金属、半导体、创新药、证券等板块的方向、关系和传导路径。规则清单可用：
...
高频事件抽取已进一步覆盖美联储鹰派/加息、财政刺激/专项债、半导体出口限制、原油供给扰动和市场制度事件；第一批影响路径已能映射到半导体、黄金、创新药、建筑建材、地产、证券、石油石化和交通运输等板块，并带有方向、关系、条件和影响周期。事件催化分已纳入来源可信度、新鲜度/重复度和证据时间衰减；事件仓库已能持久化事后窗口表现；事件雷达和板块详情已展示状态、观察点、证据可信度、影响周期和失效条件；诊断命令已能输出事件规则清单和单条事件的评分因子。
...
v2.3 起推荐结果增加生命周期跟踪。系统会把“方向判断”和“入场时机”拆开：`directionScore` 用来判断板块逻辑是否仍成立，`entryTimingScore` 用来判断当前是否适合追入。大涨但拥挤的板块会进入“过热不追”，推荐后大跌的板块会保留为“风险预警”，方向仍在但价格回落会进入“回调观察”，只有方向分转弱、动作降为减配或事件催化失效时才进入“失效移除”。这些记录会在策略跟踪页的“推荐跟踪 / 信号复盘”中展示，避免推荐名单随涨跌静默变化。

v2.4 起推荐会额外生成技术点位计划。系统会把当前价格放到支撑/阻力、均线、BOLL、ATR 和近期高低点中判断：上涨趋势优先等待回调到观察区，下跌趋势进入风险预警或逻辑失效，过热拉升保留逻辑但不追高。风险收益比低于阈值时，系统不会因为今日涨幅好就强化入场时机。
...
5. 推荐动作需要从“买/卖”升级为“观察、短线机会、趋势跟踪、长期配置、过热谨慎、回避/减配”，降低误导性；v2.3 已先落地推荐生命周期跟踪，后续还需要继续接入更完整的收益复盘。
...
5. 做“预测追踪”：每条建议生成后自动跟踪 1 日、3 日、5 日、20 日表现，用实际结果反向优化模型；v2.3 已实现生命周期状态和本地跟踪记录，后续可继续补全多窗口收益统计。
6. 做“AI 协同分析层”：让 AI 负责事件标题改写、摘要生成、影响路径解释、理由/风险去重和规则分歧提示，但不直接覆盖核心评分和最终动作；当前已接入结构化可读字段和本地 AI smoke，后续重点是把这些字段完整展示到各页面。
7. 做“UI 重构与事件工作台”：按设计稿拆分 `MainWindow`，新增事件雷达、传导路径和板块详情首屏结构。
