...
- 规则事件抽取雏形，可识别货币政策、通胀就业、商品供需和产业政策事件，并保留新闻证据；当前处在 core smoke 阶段，尚未正式接入主分析评分。
...
事件传导引擎 Phase 1 已新增 `MacroEvent`、`EventRuleBook` 和 `EventExtractionEngine`，能够把“美联储降息预期升温”“美联储将召开议息会议”“宣布降息”“CPI 高于预期导致预期修正”等文本结构化为事件类型、状态、地区、观察节点和证据。它目前先由 `InvestInsightEventSmoke` 验证，后续 Phase 3 再接入 `InsightOrchestrator` 和评分。
