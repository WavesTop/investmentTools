...
板块详情页是产品的关键决策页，后续视觉优化不能只保留事件解释。它仍需要展示投资信号、短中长期收益、核心评分、事件驱动、技术指标、阶段收益/回测、资金流、相关板块、新闻证据和数据质量，让用户既能看懂原因，也能核对原始数据。
