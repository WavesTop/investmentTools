...
- 切片 2 及之后尚未完成：新状态和新类型的实际抽取规则、结构化日历观察点、扩展路径、评分因子、事后追踪和 UI 展示仍按下方切片推进。

尚未完成：

- 财政政策、地缘贸易、金融市场制度已有模型枚举，但抽取规则尚未覆盖。
- 传闻、已发生、失效已有状态枚举，但状态解析尚未覆盖。
- 时间字段模型已具备，结构化日历解析和真实观察点填充尚未完成。
- 状态、时间和日历仍耦合在 `EventRuleBook` 的关键词判断中。
- 影响路径规则偏少，缺少加息/鹰派、财政、出口限制、油价、制度改革等高频事件。
- 评分只包含方向、强度、置信度和状态权重，缺少来源可信度、新鲜度、重复衰减和市场确认。
- 事件仓库只记录状态变化，未记录事后表现和命中率。
