...
| `src/ui/renderers/SectorDetailRenderer.cpp` | 板块详情 HTML 渲染器，覆盖投资结论、事件驱动、图表、技术指标、资金流、回测、新闻证据和数据质量。 |
